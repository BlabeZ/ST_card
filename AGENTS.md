...
本文件保存每次在仓库内工作都应遵守的稳定规则。分析统计、第三方样本和现有角色包的阶段性问题留在 `docs/analysis/`，按任务读取，不把全部分析文档一次载入上下文。
...
| `docs/analysis/cards-loli-analysis.md` | 高风险子目录的卡片形态、世界书、备用开场、脚本风险、重复和不可达条目 | 直接审计该目录或追溯专题统计时；一般结构设计优先读综合分析 |
| `docs/analysis/cards-training-analysis.md` | 成人权力交换题材、持续同意、可逆阶段、状态协议和脚本风险 | 设计成年自愿关系阶段、暂停与撤回协议或审计相关第三方内容时 |
...
- 玩家只控制 `{{user}}` 时，不替玩家补写台词、选择、同意、情绪、私人结论或关键行动。
...
- 初始化、暂停、撤回、检查点、恢复和复盘走同一套 QR/STscript 协议。
- JSON、YAML 或轻量 DSL 必须说明由谁解析，并使用正式解析器、schema 和允许路径白名单。
- 禁止用 `eval`、`new Function` 或同类机制解析模型输出。

## 内容边界

- 不迁移第三方目录中的未成年人性化、可识别现实未成年人、胁迫、羁押、监护、教师权力、所有权或生存资源控制前提。
- 涉及成人权力交换时，参与者应在身份、外观、经历和社会能力上都明确成年，不能只替换年龄数字。
- 工作、医疗、教育、监管、债务、住所和生存资源不能成为取得关系同意的手段。
- 同意必须能够暂停、撤回和结束，且不会受罚。沉默、僵住、身体反应或明确拒绝都不能被解释为同意。
- 阶段允许停滞、回退、调整和结束。数值只辅助记忆，不能自动产生关系授权。
- 人物保留关系外的职业、朋友、住所、目标和退出后的生活能力。
